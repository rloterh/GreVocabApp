...
- **[P1] OAuth connect, where the provider offers it.** **BLOCKED ON A SPIKE.** — "Sign in with…" rather than a pasted key. The keychain it would deposit a token into now exists, so the spike is the only thing in the way. → Per provider, establish whether third-party client registration is actually open to an app like this one — not assumed from the fact that an OAuth flow exists — and record the finding in [ADR 0007](./docs/adr/0007-authentication-strategy.md). Until then the installed-CLI route covers the same users without a credential ever reaching Lexicon.
