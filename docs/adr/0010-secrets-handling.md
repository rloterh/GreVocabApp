...
| 5 — custom base URL confirmed, plain `http://` refused off-loopback | **Done as logic** in `src/lib/ai/endpoint.ts`, 22 tests. Plain `http://` is refused anywhere but loopback — refused, not warned about, since a key in the clear is not something a user meant to accept. Credentials embedded in the address are refused too: they end up in logs and history. Writing the loopback check caught a live bug in my own first attempt — `/^127\./` calls `127.0.0.1.evil.test` loopback, which would have let a key travel in the clear to somebody else's host. **Still to come:** the settings screen that lets a user type one. |
