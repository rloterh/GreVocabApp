...
for (const track of TRACKS) {
  ...
  // `src/data/` is a *derived* copy of the starter months, generated by
  // scripts/sync-starters.mjs so the bundle can seed them without a fetch.
  //
  // It used to hold content of its own: GRE 1 and 2 lived only there, which
  // kept them out of the library index and made them unrecoverable once
  // unloaded. They were audited as extra words. Now the corpus is the whole
  // truth, so the copies are checked for drift rather than counted — counting
  // them would report every starter word as a duplicate of itself.
  const seed = load(SEED, new RegExp(`^${track}-\\d{2}\\.json$`));
  const corpusWords = corpus.flatMap((m) => allWordsInMonth(m.month));
  const all = corpusWords;
  perTrack.set(track, all);

  console.log(`${corpus.length} months, ${corpusWords.length} words\n`);

  const byOrdinal = new Map(corpus.map((m) => [m.month.ordinal, m.month]));
  for (const { file, month } of seed) {
    const source = byOrdinal.get(month.ordinal);
    report(
      Boolean(source),
      `${file} has a month in the corpus`,
      source ? "" : `ordinal ${month.ordinal} is bundled but not published`,
    );
    if (!source) continue;
    const matches = JSON.stringify(month) === JSON.stringify(source);
    report(
      matches,
      `${file} matches the corpus`,
      matches ? "" : "run `node scripts/sync-starters.mjs`",
    );
  }
  ...
  const ordinals = corpus.map((m) => m.month.ordinal).sort((a, b) => a - b);
  report(
  ...
  report(
    corpus.every((m) => m.month.track === track),
    "every month names its own track",
  );

  const untitled = corpus.filter(
